...
def enableAccount(user_port):
    ...
    conn.commit()
    conn.close();
    print(f"inbound with port {user_port} unblocked")
    time.sleep(2)
...
def disableAccount(user_port):
    ...
    time.sleep(2)
    os.popen("x-ui restart")
    unblocking = Timer(10, enableAccount, [user_port])
    unblocking.start()
    time.sleep(3)
...
class AccessChecker(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            netstate_data =  os.popen("netstat -antu | grep :"+str(user_port)+" | grep -v LISTEN | awk '{print $5}' | cut -d: -f1 | sort | uniq -c").read();
